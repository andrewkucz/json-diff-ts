...
export const createValue = (value: any): IComparisonEnrichedNode => ({
	...
	chain(changelist)
		...
		.map((entry) => ({
			...
			path: replace(
				entry.path,
				/(\[(?<array>\d)\]\.)/g,
				"ARRVAL_START$<array>ARRVAL_END",
			),
		}))
		.map((entry) => ({
			...entry,
			path: replace(entry.path, /(?<dot>\.)/g, ".value$<dot>"),
		}))
		.map((entry) => ({ ...entry, path: replace(entry.path, /\./, "") }))
		.map((entry) => ({
			...entry,
			path: replace(entry.path, /ARRVAL_START/g, ".value["),
		}))
		.map((entry) => ({
			...entry,
			path: replace(entry.path, /ARRVAL_END/g, "].value."),
		}))
		.value()
		.forEach((entry) => {
			switch (entry.type) {
...
export const compare = (
	...
	return applyChangelist(
		enrich(oldObject),
		flattenChangeset(diff(oldObject, newObject)),
	);
};
