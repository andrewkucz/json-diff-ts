			...
			return [
				{
					...obj,
					path:
						valueType === "Object" || path.endsWith(`[${obj.key}]`)
							? path
							: append(path, obj.key),
					valueType,
				},
	...
	switch (typeOfOldObj) {
		case "Date":
			changes = changes.concat(
				comparePrimitives(oldObj.getTime(), newObj.getTime(), path).map(
					(x) => ({
						...x,
...
const compareObject = (
	...
	const oldObjKeys = Object.keys(oldObj).filter(
		(key) => keysToSkip.indexOf(key) === -1,
	);
	const newObjKeys = Object.keys(newObj).filter(
		(key) => keysToSkip.indexOf(key) === -1,
	);
...
const removeKey = (obj: any, key: any, embeddedKey: any) => {
	if (Array.isArray(obj)) {
		...
		if (index === -1) {
			...
			console.warn(
				`Element with the key '${embeddedKey}' and value '${key}' could not be found in the array'`,
			);
			return;
		}
...
const indexOfItemInArray = (arr: any[], key: any, value: any) => {
	...
	for (let i = 0; i < arr.length; i++) {
		const item = arr[i];
		if (
			item && item[key] ? item[key].toString() === value.toString() : undefined
		) {
			return i;
		}
	}
			...
			if (subchange.value != null || subchange.type === Operation.REMOVE) {
				...
				if (change.embeddedKey === "$index") {
					...
					element = find(
						arr,
						(el) =>
							el[change.embeddedKey]?.toString() === subchange.key.toString(),
					);
				}
...
function filterExpression(
	...
	const value =
		typeof filterValue === "number" ? filterValue : `'${filterValue}'`;
